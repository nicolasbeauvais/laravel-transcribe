...
class RemoveCommandTest extends TestCase
{
    public function testCommandOutput()
    {
        ...
        $command = m::mock('\NicolasBeauvais\Transcribe\Commands\RemoveCommand[confirm]', [$manager]);
        $command->shouldReceive('confirm')->once()->with('Are you sure you want to remove "user.name"?')->andReturn(true);

        $this->app['artisan']->add($command);
        $this->artisan('transcribe:remove', ['key' => 'user.name']);

        $userENFile = (array) include $this->app['config']['transcribe.path'].'/en/user.php';
        $userNLFile = (array) include $this->app['config']['transcribe.path'].'/nl/user.php';
    ...
    public function testRemovesNestedKeys()
    {
        ...
        $this->app['artisan']->add($command);
        $this->artisan('transcribe:remove', ['key' => 'user.name.f']);
    ...
    public function testCommandOutputForVendorPackage()
    {
        ...
        $this->app['artisan']->add($command);
        $this->artisan('transcribe:remove', ['key' => 'package::file.not_found']);

        $ENFile = (array) include $this->app['config']['transcribe.path'].'/vendor/package/en/file.php';
        $SPFile = (array) include $this->app['config']['transcribe.path'].'/vendor/package/sp/file.php';
